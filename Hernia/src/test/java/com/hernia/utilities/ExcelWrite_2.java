...
public class ExcelWrite_2 {
	...
	// It looks like udemy stopped supporting uploading of .java files, this is why I have to change the  extension of the file
		...
		/*
		 * URL to get the binary - http://poi.apache.org/download.ht  ml
		 ...
		 * @params - Row num and Col num
		 */
		public  String getDateCellData(int RowNum, int ColNum) throws Exception {
			try {
				Cell = ExcelWSheet.getRow(RowNum).getCell(ColNum);
		 ...
		 * @params - Row num and Col num
		 */
		public  void setCellData(double Result, int RowNum, int ColNum) throws Exception {
			try {
				Row = ExcelWSheet.getRow(RowNum);
				Cell = Row.getCell(ColNum);
				if (Cell == null) {
